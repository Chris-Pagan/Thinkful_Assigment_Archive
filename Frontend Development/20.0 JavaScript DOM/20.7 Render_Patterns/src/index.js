...
//set the variable 
  ...
  //write the condition to show nothing  in cart
  if(books.length ===0) {
    bookHtml = "Nothing in cart"
  } else {
  //use the map method for books here
    bookHtml = books.map(renderBook).join('')
  }
  
  //call the function calculateTotal as helper her
  let total = calculateTotal();
  let totalHtml = `$${total}`
  // set two new two variables 
  let cartItems = document.querySelector("#cartItems")
  let cartTotal = document.querySelector(".total-price")
  
  cartItems.innerHTML = bookHtml
...
function main() {
  ...
  //Chris here you need to call the function sortbyprice here, that runs the render function 
    sortByPrice()
  });
  render();
};
