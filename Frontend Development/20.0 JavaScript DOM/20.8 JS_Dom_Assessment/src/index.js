...
function renderContact(contact) {
  const { id, name, username, email, picture, address:{
    street, suite, city,zipcode, geo: {lat, lng}
  }, phone, website, company } = contact;

    const content = `<div class="card" data-id=${id}>
...
function filterByCity(city) {
  //filter through contacts by city
  //returns list of contacts with city
    const filteredCities = contacts.filter(
      (contact) => contact.address.city === city
    );
...
function filterHandler() {
  //eventlistener returns name of city
  //takes name from city passed into filterBycity(), return from filterbycity is taken in by render(filteredList)
  //if no city selected, renders all cards
    const filterEl = document.querySelector("#filterOptions");
    filterEl.addEventListener("change", (e) => {
      if (e.target.value === "0") {
        render(contacts);
      } else {
        render(filterByCity(e.target.value));
      }
    });
...
function loadCities(contacts) {
  //get unique values from contacts array and populates filteroptions(filter by city dropdown) list
    const selectEl = document.querySelector("#filterOptions");
